...
def vqa_rad_test_step(pl_module, batch, output):
    """Test step for VQA-RAD: returns predictions + ground truth for metric computation."""
    id2answer = pl_module.trainer.datamodule.dm_dicts["vqa_rad"].id2answer
    vqa_logits = output["vqa_logits"]
    vqa_preds = vqa_logits.argmax(dim=-1)
    pred_strs = [id2answer[pred.item()] for pred in vqa_preds]
    # answers is a list-of-lists; each QA has exactly one gold answer
...
def _token_f1(pred: str, gold: str) -> float:
    """Token-level F1 using whitespace tokenization (matches VQA-RAD convention)."""
    ...
    rec = len(common) / len(gold_toks) if gold_toks else 0.0
    if prec + rec == 0:
        return 0.0
    return 2 * prec * rec / (prec + rec)


def vqa_rad_test_wrapup(outs, model_name, results_dir="result"):
    """Compute VQA-RAD overall / closed / open accuracy and token-F1."""
    preds, golds, types = [], [], []
    for out in outs:
        preds += out["preds"]
        golds += out["golds"]
    ...
    print(f"\n=== VQA-RAD Test Results ({model_name}) ===")
    ...
    out_path = f"{results_dir}/vqa_rad_{model_name}.json"

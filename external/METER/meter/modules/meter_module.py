...
try:
    ...
    swin_adapt_position_encoding = None
    ...
    def __init__(self, config):
        ...
        if torch.distributed.is_initialized():
            if torch.distributed.get_rank() == 0:
                if config['vit'] == 'biomedclip':
                    pass  # BiomedCLIP downloads on first use; no prefetch needed
                elif self.is_clip:
        ...
        if config['vit'] == 'biomedclip':
            from .biomedclip_adapter import BiomedCLIPVisualWrapper
            self.vit_model = BiomedCLIPVisualWrapper()
            self.is_clip = True  # patch tokens, same pooling path as CLIP
        elif self.is_clip:
